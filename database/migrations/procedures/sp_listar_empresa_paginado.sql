--select sp_listar_empresa_paginado('','','1','1','10','ref');fetch all in ref;

CREATE OR REPLACE FUNCTION public.sp_listar_empresa_paginado(
...
Begin
	...
	v_campos=' t1.id,t1.ruc,t1.razon_social,t1.direccion,t1.telefono,t1.estado ';

	v_tabla='from empresas t1 ';
	
	v_where = ' Where 1=1 and t1.id!=1 ';
	...
	If p_ruc<>'' Then
	 v_where:=v_where||' And t1.ruc like ''%'||p_ruc||'%'' ';
	End If;

	If p_razon_social<>'' Then
	 v_where:=v_where||' And t1.p_razon_social ilike ''%'||p_razon_social||'%'' '; 
	End If;	
