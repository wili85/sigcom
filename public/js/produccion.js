...
function fn_save_produccion(){
    
    //var fecha_atencion_original = $('#fecha_atencion').val();
	//var id_user = $('#id_user').val();
    $.ajax({
			url: "/produccion/send_produccion",
            type: "POST",
            //data : $("#frmCita").serialize()+"&id_medico="+id_medico+"&fecha_cita="+fecha_cita,
            ...
            success: function (result) {
					...
					location.href="/produccion/create/";
            }
    });
}
...
function AddFila(){
	...
	*/
	...
	$("#idEspecie"+ind).on("change", function (e) {
		...
		option = {
			url: '/produccion/obtener_producto_medida/' + id_persona + '/' + id_persona + '/' + id_especie,
			type: 'GET',
			dataType: 'json',
			data: {}
		};
	...
	$("#idUnidad"+ind).on("change", function (e) {
		...
		*/
		option = {
			url: '/produccion/obtener_precio_producto_medida/' + id_persona + '/' + id_persona + '/' + id_especie + '/' + id_unidad,
			type: 'GET',
			dataType: 'json',
			data: {}
		};
		$.ajax(option).done(function (data) {
			var precio = 0;
			if(data==""){
				$("#precio_especie"+ind).attr("readonly",false);
			}else{
				$("#precio_especie"+ind).attr("readonly",true);
				precio=data[0].precio;
			}
			
			$("#precio_especie"+ind).val(precio);
